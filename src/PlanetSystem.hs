...
initPlanets = do
   planets <- V.sequence $ fromList [jupiter, saturn, uranus, neptune]
...
createSun planets =
   ...
   in do
      allMomentums <- V.mapM computeMomentum planets
      let sumMomentums = V.foldl plusVect nullVect allMomentums
